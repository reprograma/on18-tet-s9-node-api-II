...
// Função getById para retornar a serie pelo ID
const getById = async(request, response) => {
    try {
        ...
        if (encontraSeriePeloId == undefined)
...
// getByTitle para retornar a serie  pelo titulo
const getByTitle = async(request, response) => {
    try {
        let seriesJson = await dbConnect();
        let tituloRequest = request.query.titulo.toLowerCase();

        let pesquisarTitulo = seriesJson.filter((serie) =>
...
const postBySerie = async(request, response) => {
...
// PUT para qualquer atualização de uma série
const updateGenerico = async(request, response) => {
    try {
        ...
        response
            .status(200)
            .json({ mensagem: "Série atualizada com sucesso!", serieEncontrada });
    } catch (error) {
        response.status(404).json({ message: error.message });
    }
};
